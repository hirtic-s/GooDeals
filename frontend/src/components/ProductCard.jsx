...
export function HeroCard({ group, isCompared, onToggleCompare }) {
  ...
  return (
    <div className="relative w-full border border-border bg-card overflow-hidden shadow-hero mb-1">
      {/* Inner glow */}
      <div className="absolute inset-0 shadow-inner-glow pointer-events-none" />

      <div className="flex flex-col md:flex-row min-h-[340px]">
        {/* Image pane */}
        <div className="relative md:w-[44%] bg-card-light flex items-center justify-center p-10 min-h-[240px]">
          {/* Series tag */}
          ...
          <img
            src={group.image}
            alt={group.baseName}
            className="object-contain h-52 w-full max-w-xs drop-shadow-2xl"
            onError={e => {
        ...
        <div className="flex-1 flex flex-col justify-between p-8 md:p-10">
          {/* Title */}
          <div>
            <p className="text-[10px] font-mono text-muted tracking-[0.2em] uppercase mb-3">
              {group.brand} / {group.category}
            </p>
            <h1 className="text-4xl md:text-5xl font-black uppercase leading-none tracking-tight text-white mb-1">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1 max-w-xs">
                  <input
                    ...
                    style={{
                      background: `linear-gradient(to right, #6cb4e4 ${pct}%, #252830 ${pct}%)`,
                    }}
                  />
                </div>
              </div>
          ...
          <div className="mt-8 flex items-center gap-6">
            <div className="flex items-center gap-4 border border-border p-3">
              ...
              <div>
                ...
                <p className="text-xl font-mono font-semibold text-white">{formatINR(selected.price)}</p>
              </div>
            </div>
            <a
            ...
            <button
              ...
              className={`text-[10px] font-mono px-4 py-2 border tracking-widest transition-all ${
                isCompared ? 'border-accent bg-accent text-[#0f1116]' : 'border-border text-muted hover:border-muted'
              }`}
            >
...
export default function ProductCard({ group, isCompared, onToggleCompare }) {
  ...
  return (
    <div className="group/card flex flex-col border border-border bg-card hover:border-border-accent transition-all duration-300 shadow-card hover:shadow-card-hover cursor-pointer">

      {/* Image container — light inner */}
      <div className="relative bg-card-light aspect-[4/3] overflow-hidden flex items-center justify-center p-6">
        {/* Inner glow */}
        <div className="absolute inset-0 shadow-inner-glow pointer-events-none" />
        <img
          src={group.image}
          alt={group.baseName}
          className="object-contain h-full w-full max-h-36 drop-shadow-xl transition-transform duration-500 group-hover/card:scale-105"
          onError={e => {
        ...
        <button
          ...
          className={`absolute top-2 right-2 w-6 h-6 border flex items-center justify-center transition-all text-[9px] font-mono ${
            isCompared
              ...
              : 'bg-card/60 border-border text-muted hover:border-accent hover:text-accent'
          }`}
        >
          {isCompared ? '✓' : '+'}
        </button>
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        {/* Name + color */}
        <div>
          <h3 className="text-xs font-bold uppercase tracking-wider text-white leading-tight line-clamp-2">
            {group.baseName}
          </h3>
        ...
        {maxIdx > 0 ? (
              ...
              {group.storageOptions.map((opt, i) => (
                <button
                  ...
                  className={`text-[9px] font-mono tracking-wider transition-colors ${
                    i === sliderIdx ? 'text-accent' : 'text-muted hover:text-white'
                  }`}
                >
                  {opt.storage ?? '—'}
                </button>
          ...
          <span className="text-[9px] font-mono text-muted border border-border px-1.5 py-0.5 w-fit tracking-widest">
            {selected.storage}
          </span>
        ) : null}
