...
/**
 * Sidebar filter panel — fully dynamic, data-driven.
 * Each section only renders when its available options array is non-empty.
 */
export default function Sidebar({
  ...
  const hasAnyDynamicFilters =
    availableBrands.length > 0 || availableStorages.length > 0 || availableRams.length > 0;
  ...
  return (
    <aside className="bg-paper dark:bg-card border border-gray-200 dark:border-border rounded-none p-5 flex flex-col gap-6 h-fit sticky top-20 w-64 flex-shrink-0">
        ...
        <div className="flex flex-col gap-2">
          {['Amazon', 'Flipkart'].map(store => {
            ...
            return (
              <button
                key={store}
                onClick={() => onChange({ ...filters, stores: toggleArr(stores, store) })}
              ...
              >
                <span
                  className="w-3.5 h-3.5 flex items-center justify-center border transition-colors duration-200 flex-shrink-0"
      ...
      {hasAnyDynamicFilters ? (
        ...
        <p className="text-[10px] font-mono text-muted tracking-[0.2em] text-center py-2">
